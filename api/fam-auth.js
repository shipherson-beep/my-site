// api/fam-auth.js — обмін Google ID-токена на серверну сесію (для застосунку «Лад»).
//
// POST /api/fam-auth  { credential: "<Google ID token>" }
//   → 200 { token, email, name, picture }   (token — сесія на 30 днів)
//   → 401 якщо акаунт не дозволений або токен недійсний
//
// Див. api/_fam-auth.js щодо потрібних змінних середовища (AUTH_SECRET тощо).

import { verifyGoogleIdToken, newSession } from './_fam-auth.js';
