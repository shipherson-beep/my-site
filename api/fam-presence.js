// api/fam-presence.js — «хто зараз у застосунку» (живі сесії родини).
//
// POST /api/fam-presence  { name?, picture? }  → пульс: оновлює last_seen і повертає
//      { users: [{email,name,picture,lastSeen}] } — усіх, кого бачили за останні ACTIVE_SEC секунд.
// GET  /api/fam-presence  → той самий список без оновлення власного пульсу.
//
// Захищено серверною сесією (Authorization: Bearer <token>). Окрема таблиця fam_presence
// (не торкається ні даних задач, ні бюджету).
//
// Потрібні env: DATABASE_URL (+ автентифікація, див. _fam-auth.js).
...
function getSql() {
  ...
  if (!_ready) {
    _ready = sql`
      CREATE TABLE IF NOT EXISTS fam_presence (
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      await sql`
        INSERT INTO fam_presence (email, name, picture, last_seen)
