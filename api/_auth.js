// api/_auth.js — спільна бібліотека автентифікації (НЕ є HTTP-роутом: Vercel
// ігнорує файли в /api, що починаються з підкреслення).
//
// Модель доступу:
//   1. Клієнт входить через Google → отримує Google ID-токен.
//   2. POST /api/auth { credential } → сервер перевіряє токен у Google,
//      звіряє email зі списком дозволених і видає власну СЕСІЮ —
//      підписаний HMAC-токен на 30 днів (не залежить від 1-годинного
//      терміну Google-токена, тож фонова синхронізація не ламається).
//   3. /api/data і /api/parse приймають цю сесію в заголовку
//      Authorization: Bearer <token>. Код "2109" більше не використовується.
//
...
//   BUDGET_ALLOWED_EMAILS  — дозволені акаунти через кому. Необов'язково (є дефолт).
...
export function allowedEmails() {
  const env = process.env.BUDGET_ALLOWED_EMAILS;
...
// Перевіряє, що запит — від дозволеного користувача з валідною сесією.
...
// Повертає { email, name, picture } або null.
